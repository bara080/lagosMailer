...
export async function sendMany(mailer, provider, messages, { batchSize = 100 } = {}) {
  ...
  const sendOne = async (i) => {
    try { const res = await mailer.send(messages[i]); results[i] = { ok: true, id: res?.id || null }; }
    catch (err) { results[i] = { ok: false, error: String(err?.message || err).slice(0, 500) }; }
  };
  ...
  for (let start = 0; start < messages.length; start += batchSize) {
    ...
    try {
      ...
      // Batch call failed as a whole → retry this slice one-by-one so a single bad
      // message is isolated instead of failing the entire slice.
