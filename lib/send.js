...
export async function runBlast({ ids, subject, html, text, dryRun }) {
  ...
  store.logActivity({ type: 'send', text: `Blast "${label}" started — ${targets.length} recipient(s)` });
  ...
  try {
    for (const l of targets) {
      ...
      try {
        ...
        store.markContacted(l.id, subj);
        store.logSend({ to: l.email, subject: subj, status: 'sent', source: l.source });
        results.push({ id: l.id, email: l.email, status: 'sent' });
        sent++;
      } catch (e) {
        store.logSend({ to: l.email, subject: render(subject, l), status: 'failed', source: l.source });
  ...
  store.setLastBlast(summary);
  store.logActivity({ type: 'done', text: `Blast "${label}" completed — ${sent} sent, ${summary.failed} failed` });
...
export async function sendCampaign(id, { dryRun }) {
  ...
  const targets = store.resolveAudience(camp.audience);
  const cfg = smtpConfig();
  const results = [];
  ...
  store.logActivity({ type: 'send', text: `Campaign "${camp.name}" started — ${targets.length} recipient(s)` });
  ...
  try {
    for (const l of targets) {
      ...
      try {
        ...
        store.logSend({ to: l.email, subject: subj, status: 'failed', source: l.source });
        results.push({ id: l.id, email: l.email, status: 'failed', error: e.message });
      }
    }
  } finally {
  ...
  store.logActivity({ type: 'done', text: `Campaign "${camp.name}" completed — ${sent} sent, ${failed} failed` });
