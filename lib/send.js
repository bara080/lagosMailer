...
export function renderSignatureHtml(sig, lead) {
  ...
  const links = Object.keys(SOCIAL_LABELS)
    .map((k) => {
      ...
      if (!u) return '';
      return `<a href="${esc(u)}" style="color:#6b7280;text-decoration:none;font-weight:600;display:inline-block;margin-right:16px"><img src="${APP_URL}/social-${k}.png" width="16" height="16" alt="${SOCIAL_LABELS[k]}" style="vertical-align:middle;margin-right:5px;border:0" />${SOCIAL_LABELS[k]}</a>`;
    })
    .filter(Boolean).join('');
...
export async function sendCampaignBatch(company, id, { dryRun, size = BATCH_SIZE } = {}) {
  ...
  const batch = queue.slice(0, size); // target objects
  ...
  const emailer = await Emailer.open({ ...cfg, from: camp.fromName ? `${camp.fromName} <${fromAddr}>` : fromAddr });
  try {
    for (const l of batch) {
      ...
      try {
        await emailer.send({
          ...
          html: camp.html ? render(camp.html, l) + att.inlineHtml + renderSignatureHtml(sig, l) : undefined,
  ...
  if (interrupted === 'stop') {
    await store.clearControl(co, id);
    await store.updateCampaign(co, id, { status: 'stopped', queue: null, sent: totalSent, delivered: totalSent, sentTo: sentToArr });
  ...
  if (interrupted === 'pause') {
    await store.updateCampaign(co, id, { status: 'paused', queue: remainingQueue, sent: totalSent, delivered: totalSent, sentTo: sentToArr });
